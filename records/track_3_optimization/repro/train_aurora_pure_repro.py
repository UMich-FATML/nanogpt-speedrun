...
def aurora_orthogonalize(G: Tensor, pp_iterations: int = 2, pp_beta: float = 0.5,
    ...
    """Leverage-uniform polar from tilde-research/aurora-release."""
...
@torch.compile
def aurora_update(grad, momentum, mu=0.95, nesterov=True, pp_iterations=2, pp_beta=0.5):
...
class Aurora(torch.optim.Optimizer):
    def __init__(self, params, lr=0.05, weight_decay=0.025, mu=0.95,
        ...
        for group in self.param_groups:
            ...
            for base_i in range(0, len(params), world_size):
                if base_i + rank < len(params):
                    ...
                    update = aurora_update(p.grad, state["momentum"], mu=group["mu"],
...
for _ in range(num_trials):
    ...
    optimizer2 = Aurora([p for p in model.blocks.parameters() if p.ndim >= 2],
                        lr=0.05, weight_decay=0.025,
                        pp_iterations=2, pp_beta=0.5)
